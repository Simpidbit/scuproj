...
simpid::SocketAbstract::brecv(size_t length)
{
  unsigned char buffer[length];
  int n = this->brecv(buffer, length);

  std::vector<unsigned char> msg;
  msg.resize(n);
  memcpy(&msg[0], buffer, n);

  return msg;
}

std::vector<unsigned char>
simpid::SocketAbstract::brecvall()
{
  std::vector<unsigned char> msg;
  std::list<unsigned char *> buffer_list;

  buffer_list.emplace_back(new unsigned char[BUFSIZ + 1]);
  memset(buffer_list.back(), 0, BUFSIZ + 1);

  int count = 0;
  for (;;) {
    count = this->brecv(buffer_list.back(), BUFSIZ);
    if (count == BUFSIZ) {
      buffer_list.emplace_back(new unsigned char[BUFSIZ + 1]);
      memset(buffer_list.back(), 0, BUFSIZ + 1);
    } else break;
  ...
  msg.resize((buffer_list.size() - 1) * BUFSIZ + count + 1);

  size_t offset = 0;
  for (auto bufptr : buffer_list) {
    if (bufptr == buffer_list.back()) {
      memcpy(&msg[offset], bufptr, count);
      msg.back() = 0;
      break;
    }
    memcpy(&msg[offset], bufptr, BUFSIZ);
    offset += BUFSIZ;
  }

  for (auto bufptr : buffer_list) delete[] bufptr;
  return msg;
}

int
simpid::SocketAbstract::brecv(unsigned char *buf, size_t length)
{
    // buf长度 >= length + 1
    int tmp = 0;
    memset(buf, 0, length);
